...
{
    public class TracerControlChartController : Controller
    {
        ...
        public ActionResult Index(int id, int? actionType)
        {
            ...
            HelperClasses.SetReportOrScheduleID(id, (int)ReportsListEnum.TracerControlChart);
            ...
            if (AppSession.ReportScheduleID > 0)
            {
                ...
                return View(reportservice.GetSearchListsForSavedParameters(AppSession.ReportScheduleID, savedParameters, WebConstants.TRACER_REPORT_TITLE_TRACER_CONTROL_CHART_REPORT));
            }
            else
                return View(reportservice.GetSearchLists(WebConstants.TRACER_REPORT_TITLE_TRACER_CONTROL_CHART_REPORT));
            }
            catch (Exception ex)
            {
        ...
        public PartialViewResult _TracerControlChart(Search search, Email emailInput)
        {
            ReportViewer reportViewer = new ReportViewer();
            try
            {
                var dcaService = new TracerControlChart();
                ...
                reportViewer = dcaService._TracerControlChartRDLC(search, emailInput);
