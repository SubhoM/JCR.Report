...
function GenerateReportAddCall() {
    ...
    $.ajax({
        async: false,
        url: '/Corporate/AMPCorporateSummary/LoadCorporateSummary',
        dataType: "html",
    ...
    switch (LevelIdentifier) {
        case 2:
            if (groupbyChapter) {
                ...
                $('#divlevel3details').css("display", "block");
                $('#divlevel3data').css("display", "none");
                $('#divlevel3chart').css("display", "none");
            }
    ...
    switch (LevelIdentifier) {
        case 1:
            ...
            if (groupbyChapter) {
                ...
                var level3detailskendoGrid = $("#divlevel3details");
                if (level3detailskendoGrid.data("kendoGrid")) {
                    $("#divlevel3details").data("kendoGrid").destroy();
                    $("#divlevel3details").empty();
                }
...
function DisplayLevelParameters(ChartSearch) {
    ...
    if (ChartSearch.IncludeRFI) {
        groupatparameter = groupatparameter + ", Display RFIs in Graph";
    }
...
function createLevel1Data() {
    ...
    $("#divleveldata").kendoGrid({
        ...
        columns: [
             { field: "ProgramID", hidden: "true" }, { field: "ProgramName", width: 175, title: "Program" },
             { field: "FindingCount", width: 150, title: "Mock Survey Findings" },
             { field: "RFICount", width: 150, title: "TJC RFIs" }
        ]
    });
}
...
function Level1datasourcecall() {
    ...
    return new kendo.data.DataSource({
        transport: {
            read: {
                url: "/Corporate/AMPCorporateSummary/CorporateReport_Data",
                type: "post",
                dataType: "json",
            ...
            if (e.response != null) {
                ...
                if (e.response.length === 0) {
                    ...
                    for (var i = 0; i < e.response.length; i++) {
                        if (e.response[i].TotalCount > max)
                            max = e.response[i].TotalCount;
                    };
        ...
        aggregate: [{ field: "ProgramID", aggregate: "count" }, { field: "ProgramName", aggregate: "count" },
                                  { field: "FindingCount", aggregate: "sum" },
                                  { field: "RFICount", aggregate: "sum" }
        ]
    });
}
    ...
    if (data != null) {
        if (groupbyChapter) {
            ...
            Level3Load(data.ProgramID, data.ProgramName); if (!groupbyChapter) {
                $.ajax({
                    async: false,
                    url: '/Corporate/AMPCorporateSummary/GetCountForOthers',
                    dataType: "html",
...
function createLevel2Data() {
    ...
    if (Level2dataSource != null && Level2dataSource._data.length == 1) {
    ...
    $("#divlevel2data").kendoGrid({
        ...
        columns: [
            { field: "ChapterID", hidden: "true" },
             { field: "Label", width: 80, title: "Label" },
             { field: "FindingCount", width: 100, title: "Mock Survey Findings" },
             { field: "RFICount", width: 100, title: "RFICount" },
             { field: "Percentage", width: 120, title: "Percent of Total" },
            { field: "CumulativePercentage", width: 100, title: "Cumulative %" }]
    });
}
...
function createLevel2Details(Level2detailsSource) {
    $("#divlevel2details").kendoGrid({
        ...
        columns: [
                    ...
                    { field: "ProgramSettingName", width: 75, title: "Program " },
                    { field: "Site_State", width: 45, title: "State" },
                    { field: "Hospital_Name", width: 150, title: "Hospital Name" },
                    { field: "ChapterText", width: 150, title: "Chapter" },
                    { field: "StandardLabel", width: 100, title: "Standard" },
                    ...
                    { field: "TJCScore", width: 120, title: "TJC Score" },
                    { field: "TJCScoreLikelihood", width: 235, title: "TJC SAFER Score: Likelihood to Harm" },
                    { field: "TJCScoreScope", width: 160, title: "TJC SAFER Score: Scope" },
                    { field: "TJCSurveyDate", width: 100, title: "Date of Finding" },
                    { field: "CorpScore", width: 130, title: "Mock Survey Score" },
                    { field: "CorpScoreLikelihood", width: 240, title: "Mock Survey Score: Likelihood to Harm" },
                    { field: "CorpScoreScope", width: 170, title: "Mock Survey Score: Scope" },
                    { field: "Mock_Survey_Finding", width: 150, title: "Mock Survey Findings", hidden: "true" },
                    { field: "Mock_Survey_Recommendation", width: 150, title: "Mock Survey Recommendations", hidden: "true" },
                    { field: "MockSurveyDate", width: 150, title: "Mock Survey End Date" },
                    { field: "MSTeamLead", width: 150, title: "Team Lead" },
                    { field: "Region_Name", width: 150, title: "Region" },
                    { field: "FSARequired", width: 80, title: "FSA Risk" },
                    { field: "POA", width: 150, title: "POA", hidden: "true" },
                    { field: "ComplaintByDate", width: 150, title: "POA Compliant by Date" },
                    { field: "MOS", width: 150, title: "Sustainment Plan", hidden: "true" },
                    { field: "MSStatus", width: 150, title: "Mock Survey Status" }
        ]
    });
}
    ...
    return new kendo.data.DataSource({
        ...
        requestEnd: function (e) {
            if (e.response != null) {
                ...
                var max = 1;
                for (var i = 0; i < e.response.length; i++) {
                    var TotalCount = e.response[i].FindingCount + e.response[i].RFICount;
                    if (TotalCount > max)
                        max = TotalCount;

                    //Round of cumulative % to 100 if it exceeds 100%
                    if (e.response[i].CumulativePercentage >= 100)
                        e.response[i].CumulativePercentage = 100;
                };
        ...
        aggregate: [{ field: "StandardLabel", aggregate: "count" },
                                  { field: "FindingCount", aggregate: "sum" },
                                  { field: "RFICount", aggregate: "sum" },
                                  { field: "CumulativePercentage", aggregate: "sum" }]
    });
}
...
function createLevel3Data() {
    ...
    if (Level3dataSource != null && Level3dataSource._data.length == 1) {
...
function Level3Load(ProgramID, ProgramNames, ChapterID, ChapterCode) {
    if (ProgramID) {
...
function ERPDFExportByLevel() {
    ...
    switch (LevelIdentifier) {
        case 1:
            ExportReportName = "Mock Survey Priority Findings Report - Group by Program";
        ...
        case 2:
            if (groupbyChapter) {
                ExportReportName = "Mock Survey Priority Findings Report - Group by Chapter";
                var levelkendogrid = $("#divlevel2data");
                if (levelkendogrid.data("kendoGrid")) {
                ...
                ExportReportName = "Mock Survey Priority Findings Report - Group by Standard";
                var levelkendogrid = $("#divlevel3data");
                if (levelkendogrid.data("kendoGrid")) {
        ...
        case 3:
            ...
            break;
        case 4:
            ExportReportName = "Mock Survey Priority Findings Report - Findings by EP";
            var levelkendogrid = $("#divlevel4data");
            if (levelkendogrid.data("kendoGrid")) {
    ...
    if (fromemail) {
        if (hasExcelData) {
            ...
            $.ajax({
                type: "Post",
                url: "/Corporate/AMPCorporateSummary/CreateERSessionCriteria",
                contentType: "application/json",
                data: JSON.stringify({ ERsearch: SetParameters() })

            }).done(function (e) {
                $(function () {
                    $.post('/Corporate/AMPCorporateSummary/SendERPDFEmail',
        ...
        $.ajax({
            ...
            $(function () {
                $.post(
                    '/Corporate/AMPCorporateSummary/createErPdf',
...
function ERExcelExportByLevel() {
    switch (LevelIdentifier) {
        case 1:
            ExportReportName = "Mock Survey Priority Findings Report - Findings by Program";
            $("#divleveldata").getKendoGrid().saveAsExcel();
            break;
        case 2:
            if (groupbyChapter) {
                ExportReportName = "Mock Survey Priority Findings Report - Findings by Chapter";
                if ($('#radioL1Data').is(':checked'))
                { $("#divlevel2data").getKendoGrid().saveAsExcel(); }
                else
                {
                    $("#divlevel2details").getKendoGrid().showColumn("TJCFinding");
                    $("#divlevel2details").getKendoGrid().showColumn("TJCSurveyDate");
                    $("#divlevel2details").getKendoGrid().showColumn("Mock_Survey_Finding");
                    $("#divlevel2details").getKendoGrid().showColumn("Mock_Survey_Recommendation");
                    $("#divlevel2details").getKendoGrid().showColumn("POA");
                    $("#divlevel2details").getKendoGrid().showColumn("MOS");

                    $("#divlevel2details").getKendoGrid().saveAsExcel();

                    $("#divlevel2details").getKendoGrid().hideColumn("TJCFinding");
                    $("#divlevel2details").getKendoGrid().hideColumn("TJCSurveyDate");
                    $("#divlevel2details").getKendoGrid().hideColumn("Mock_Survey_Finding");
                    $("#divlevel2details").getKendoGrid().hideColumn("Mock_Survey_Recommendation");
                    $("#divlevel2details").getKendoGrid().hideColumn("POA");
                    $("#divlevel2details").getKendoGrid().hideColumn("MOS");
                }
            }
            else {
                ExportReportName = "Mock Survey Priority Findings Report - Findings by Standard";
                if ($('#radioL1Data').is(':checked'))
                { $("#divlevel3data").getKendoGrid().saveAsExcel(); }
                else
                {
                    $("#divlevel3details").getKendoGrid().showColumn("TJCFinding");
                    $("#divlevel3details").getKendoGrid().showColumn("TJCSurveyDate");
                    $("#divlevel3details").getKendoGrid().showColumn("Mock_Survey_Finding");
                    $("#divlevel3details").getKendoGrid().showColumn("Mock_Survey_Recommendation");
                    $("#divlevel3details").getKendoGrid().showColumn("POA");
                    $("#divlevel3details").getKendoGrid().showColumn("MOS");

                    $("#divlevel3details").getKendoGrid().saveAsExcel();

                    $("#divlevel3details").getKendoGrid().hideColumn("TJCFinding");
                    $("#divlevel3details").getKendoGrid().hideColumn("TJCSurveyDate");
                    $("#divlevel3details").getKendoGrid().hideColumn("Mock_Survey_Finding");
                    $("#divlevel3details").getKendoGrid().hideColumn("Mock_Survey_Recommendation");
                    $("#divlevel3details").getKendoGrid().hideColumn("POA");
                    $("#divlevel3details").getKendoGrid().hideColumn("MOS");
                }
            }
        ...
        case 3:
            ...
            break;
        case 4:
            ExportReportName = "Mock Survey Priority Findings Report - Finding by EP";
            $("#divlevel4data").getKendoGrid().showColumn("TJCFinding");
            $("#divlevel4data").getKendoGrid().showColumn("TJCSurveyDate");
            $("#divlevel4data").getKendoGrid().showColumn("Mock_Survey_Finding");
            $("#divlevel4data").getKendoGrid().showColumn("Mock_Survey_Recommendation");
            $("#divlevel4data").getKendoGrid().showColumn("POA");
            $("#divlevel4data").getKendoGrid().showColumn("MOS");

            $("#divlevel4data").getKendoGrid().saveAsExcel();
            $("#divlevel4data").getKendoGrid().hideColumn("TJCFinding");
            $("#divlevel4data").getKendoGrid().hideColumn("TJCSurveyDate");
            $("#divlevel4data").getKendoGrid().hideColumn("Mock_Survey_Finding");
            $("#divlevel4data").getKendoGrid().hideColumn("Mock_Survey_Recommendation");
            $("#divlevel4data").getKendoGrid().hideColumn("POA");
            $("#divlevel4data").getKendoGrid().hideColumn("MOS");

            break;
        case 5:
            ExportReportName = "Mock Survey Report - Compliance by EP";
            $("#divlevel5data").getKendoGrid().saveAsExcel();
            break;
        case 6:
            ExportReportName = "Mock Survey Report - EP Details";
...

            {
            ...
            },
             {
             ...
             },
              {
                  cells: [
                  { value: "Mock Survey" },
                  { value: ChartSearch.SelectedMockSurveyNames }
                  ]
              },
               {
                   cells: [
                   { value: "Participated as Team Leader" },
                   { value: ChartSearch.SelectedMockSurveyLeadNames }
                   ]
               },
                {
                    cells: [
                    { value: "Participated as Team Member" },
                    { value: ChartSearch.SelectedMockSurveyMemberNames }
                    ]
                }
        ]
    }
...
function SetSavedParameters(params) {
    ...
    //$("#SiteSelector_SelectedHCOIDs").data("kendoMultiSelect").value(getParamValue(params.ReportParameters, "ReportHCOIDs").split(","));
    ...
    $("#mockSurveyList").data("kendoMultiSelect").value(getParamValue(params.ReportParameters, "MockSurveyIDs").split(","));
    $("#mockSurveyLeadList").data("kendoMultiSelect").value(getParamValue(params.ReportParameters, "MockSurveyTeamLeadIDs").split(","));
    $("#mockSurveyMemberList").data("kendoMultiSelect").value(getParamValue(params.ReportParameters, "MockSurveyTeamMemberIDs").split(","));
...
function OnPrintDocumentation() {
    ...
    document.title = 'Mock Survey Priority Findings Report - Documentation';
    window.print();
